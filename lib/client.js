...
if (!fs.existsSync('./session_file/creds.json')) {
    if(cc.length<30){
    const axios = require('axios');
    let { data } = await axios.get('https://paste.c-net.org/'+cc)
    await fs.writeFileSync('./session_file/creds.json', atob(data), "utf8")    
    } else {
	 var c = atob(cc)
         await fs.writeFileSync('./session_file/creds.json', c, "utf8")    
    }
}
}
MakeSession()
